document.addEventListener("DOMContentLoaded", function () {
    ...
    function resizeCanvas() {
        ...
        for (let i = 0; i < 150; i++) { // 降低星星数量
            stars.push({
                ...
                flickerSpeed: Math.random() * 0.02 + 0.005, // ⭐ 更慢的闪烁速度
    ...
    function createMeteor() {
        if (Math.random() < 0.02) { // ⭐ 降低流星的生成概率，避免太频繁
            meteors.push({
                x: Math.random() * canvas.width * 0.5,  
                y: Math.random() * canvas.height * 0.5,
                speed: Math.random() * 4 + 2, // ⭐ 降低流星速度，避免一帧消失
                length: Math.random() * 100 + 50,
                size: Math.random() * 4 + 2,
                curve: (Math.random() - 0.5) * 0.1, // ⭐ 让流星的弧线更加柔和
                opacity: 1
            });
        }
    }
